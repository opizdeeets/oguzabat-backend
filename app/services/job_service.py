...
from app.models.models import Vacancy, EmploymentType
...
async def create_vacancy(db: AsyncSession, vacancy_in: VacancyCreate) -> Vacancy:
    try:
        ...
        print(f"Creating vacancy: {db_vacancy}")
        db.add(db_vacancy)
        await db.commit()
...
async def get_vacancies(db: AsyncSession, employment_type: Optional[EmploymentType] = None) -> List[Vacancy]:
    try:
        stmt = select(Vacancy)
...
async def delete_vacancy(db: AsyncSession, vacancy_id: int) -> bool:
    try:
        result = await db.execute(delete(Vacancy).where(Vacancy.id == vacancy_id))  # Эффективное удаление
        await db.commit()
